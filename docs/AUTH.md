...
| **موافقة OAuth** | `MCP_OAUTH_APPROVAL_PASSWORD` | صفحة إدخال كلمة مرور قبل إكمال الاتصال (اختياري) |
...
### كلمة مرور الموافقة (اختياري) ✓ مُختبر
...
**Docker:** أضف المتغير في `.env` وضمن `docker-compose.yml` (مُضمّن افتراضياً). أعد تشغيل الحاويات بعد الإضافة.
...
3. **إذا كان `MCP_OAUTH_APPROVAL_PASSWORD` معيّناً:** تظهر صفحة لإدخال كلمة المرور. أدخلها ثم اضغط Approve.
4. **Hub يصدر كوداً مؤقتاً** ويُعيد التوجيه إلى Claude.
5. **Claude يتبادل الكود مع Hub** عبر `/oauth/token` (اتصال سيرفر-إلى-سيرفر).
6. **Hub يعيد التوكن** (`MCP_HOST_TOKEN`) كـ access token لـ Claude.
7. **Claude يحفظ التوكن** ويستخدمه في كل طلبات MCP التالية.
...
- **كلمة مرور الموافقة:** عند تعيين `MCP_OAUTH_APPROVAL_PASSWORD`، لا يمكن إكمال الاتصال إلا لمن يعرفها.
